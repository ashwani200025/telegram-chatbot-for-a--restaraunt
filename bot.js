...
const bot = new Telegraf('6138895517:AAEGO3ANuEuPb8q3EohX6BeBde74oMJK-LQ');
...
bot.command('start', ctx => {
    ...
    bot.telegram.sendMessage(ctx.chat.id, 'Welcome to my store! Please type "hello" ! 10 inventory items we have with us which can be accessed by typing the code of product example : type "001", similarly type "002" etc.', {
...
bot.hears('hello', ctx => {
    console.log(ctx.from)
    bot.telegram.sendMessage(ctx.chat.id, 'Explore orders for new customer ', {
        reply_markup: {
            inline_keyboard: [
                [
...
bot.hears('001', ctx => {
    console.log(ctx.from)
    let priceMessage = `Product name : Drinks , Price : Rs. 499, remaining quantity : 100, weight : 500grams, type : grocery . . . Please type for example pine , apple, berry, orange etc. `;
    ctx.deleteMessage();
    bot.telegram.sendMessage(ctx.chat.id, priceMessage, {
        reply_markup: {
            inline_keyboard: [
